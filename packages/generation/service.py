...
class CitationOut(BaseModel): source_id:str; document_name:str; page:int|None=None; chunk_id:str; quote:str
class AnswerOut(BaseModel): answer:str; citations:list[CitationOut]; confidence:float; grounded:bool; needs_human_review:bool; follow_up_questions:list[str]=[]; warnings:list[str]=[]
class GenerationService:
    def generate(self, question:str, chunks:list, threshold=.55)->AnswerOut:
        warnings=[]
        if inspect_text(question)['pii']: warnings.append('PII detected in user input and redacted for logs')
        ...
        top=safe[0]; conf=max(0.1,min(0.95,top.score)); quote=top.text[:180]
